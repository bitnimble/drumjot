...
import { px } from 'src/editing/viewport/view_config';
 ...
 * extents, and the gutter width. Reads {@link JotEditorStore} only to
...
export class ViewportPresenter {
  ...
  constructor(viewport: ViewportStore, jotEditorStore: JotEditorStore) {
    this.viewport = viewport;
    this.jotEditorStore = jotEditorStore;
    makeAutoObservable(this, { viewport: false, jotEditorStore: false });
  }

  setZoom(z: number) {
    const clamped = Math.max(MIN_ZOOM, Math.min(MAX_ZOOM, z));
    this.viewport.zoom = clamped;
    this.jotEditorStore.viewConfig.barWidth = px(BASE_BAR_WIDTH * clamped);
  }
