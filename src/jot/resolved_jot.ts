 ...
 *
 * `RenderedJot` is now a FAÇADE over the reactive Jot document: its
 * constructor converts the DSL `source` into a reactive model
 * ({@link dslToReactive}) and derives `.structure` / `.resolved` /
 * colours / pixels from the domain stores (structure / palette / layout),
 * mapping their `Struct*` output back onto the legacy `Structural*` /
 * `Resolved*` shapes consumers expect. Tempo, timeline and the drum-offset
 * transform still read `this.source` + `this.structure` and reuse the
 * existing helpers. The DSL `source` is kept only for metadata + tempo +
 * the synthesised `note.source`; a follow-up pass decomposes this façade
 * into the domain stores/presenters directly. Re-exported from `jot.ts`.
 */
import { action, computed, makeObservable, observable } from 'mobx';
import { computedFn } from 'mobx-utils';
import { Bar, Instrument, Jot, Metadata, Modifier, Note, Sticking, TimeSignature, Voice, Volume } from 'src/dsl/dsl';
...
// ---------- RenderedJot: reactive-backed façade ----------
...
export class RenderedJot {
  ...
  constructor(public source: Jot, viewConfig?: ViewConfig) {
    ...
    // Explicit annotations: only the public derived views are computed and
    // the offset fields observable; the private mapping helpers stay plain
    // methods so their reads are tracked by the computed that calls them.
    makeObservable(this, {
