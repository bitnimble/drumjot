...
import { AudioTrackId, AudioTrackRole } from 'src/jot_view/playback/audio_tracks';
...
import { JotViewStore, buildJotModel } from './jot_view_store';
...
/**
 * Song-load orchestration over {@link JotViewStore}: the file loaders
...
export class JotViewPresenter {
   ...
   * loading counter / label live on {@link JotViewStore}). Errors
  ...
  async withLoading<T>(label: string, fn: () => Promise<T>): Promise<T> {
    runInAction(() => {
      if (this.jotViewStore.loadingCount === 0) this.jotViewStore.loadingLabel = label;
      this.jotViewStore.loadingCount += 1;
    });
    try {
      return await fn();
    } finally {
      runInAction(() => {
        this.jotViewStore.loadingCount -= 1;
        if (this.jotViewStore.loadingCount === 0) this.jotViewStore.loadingLabel = undefined;
      });
    }
  }
   ...
   * {@link JotViewStore} atomically. The sole place the four song fields
  ...
  private installJot(source: Jot | undefined): void {
    if (!source) {
      this.jotViewStore.source = undefined;
      this.jotViewStore.structural = undefined;
      this.jotViewStore.palette = undefined;
      this.jotViewStore.tempo = undefined;
      return;
    }
    const { structural, palette, tempo } = buildJotModel(source, this.jotViewStore.viewConfig);
    this.jotViewStore.source = source;
    this.jotViewStore.structural = structural;
    this.jotViewStore.palette = palette;
    this.jotViewStore.tempo = tempo;
  }
  ...
  setJot(source: Jot | undefined) {
    ...
    this.jotViewStore.currentExampleId = undefined;
  ...
  setExamples(examples: readonly ExampleJot[]) {
    this.jotViewStore.examples = examples;
  }

  loadExample(id: string) {
    const example = this.jotViewStore.examples.find((e) => e.id === id);
    if (!example) return;
    this.installJot(example.jot);
    this.jotViewStore.currentExampleId = id;
    this.provenancePresenter.clearNoteProvenance();
    this.lyricsPresenter.clearLyrics();
    jotPlayer.stop();
  }
