...
import { SettingsPresenter } from './settings_presenter';
import { MixerPresenter } from './mixer_presenter';
import { ProvenancePresenter } from './provenance_presenter';
import { LyricsPresenter } from './lyrics_presenter';

/**
 * Song-load orchestration over {@link DocumentStore}: the file loaders
 * (.jot / MIDI / ParaDB / debug bundle / lyrics / audio track), the
 * example picker, and the shared debug-bundle apply path. The sole writer
 * of `currentJot` / `currentExampleId` and the loading-overlay counter.
 *
