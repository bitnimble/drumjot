...
The **Android app** reuses the same Tauri shell + web frontend but has **no
sidecar** (no on-device Python/ML): it transcribes over the HTTP backend, same
as the web build, against the server URL set in Settings -> Advanced. The
desktop-only Rust (sidecar, capability install, portable paths) is
`#[cfg(desktop)]`-gated; `gen/android` is generated by `bun run android:init`
and gitignored. See the spec's "Android" section.
...
**Android** (`bun`, needs `ANDROID_HOME` + `NDK_HOME` + JDK 17, and the
`aarch64-linux-android` + `armv7-linux-androideabi` Rust targets):
`bun run android:init` (one-time, regenerates the gitignored `gen/android`),
`bun run android:dev` (live-reload onto a device/emulator), `bun run
android:build` (universal APK over the two ABIs). Both run/build scripts pass
`--config '{"bundle":{"resources":[]}}'` to drop the desktop-only bundled
Python/uv resources, which the Android build would otherwise demand (the
platform-config deep-merge can't clear them; only `--config`'s replace-merge
...
- **Webview logic** (the bulk): `bun run test` (unit) + `bun run e2e` (Playwright,
  ...
  interface; the `TauriBridge` invoke/Channel paths via `@tauri-apps/api/mocks`
  (`src/desktop/desktop_bridge.test.ts`).
- **Rust commands**: `cargo test` (`src-tauri/`), using `tauri::test::mock_builder`
  (the `test` dev-dep feature). Commands are generic over `R: Runtime` so the
  mock runtime can drive them; the sidecar broker test runs a fake sidecar via
  `DRUMJOT_SIDECAR_PYTHON` and captures forwarded frames through a `Channel`.
- **Real desktop shell**: `bun run e2e:tauri`, WebdriverIO + `@wdio/tauri-service`
  driving the actual WebKitGTK webview (the engine Playwright's Chromium can't
  cover). Needs a display (`xvfb-run -a bun run e2e:tauri` on a headless box).
  The build passes `tauri/custom-protocol` (serve embedded assets, not the dev
  URL) + the `wdio` cargo feature (the in-app Execute API + embedded WebDriver
  server, both optional/feature-gated → never in a shipped build). Specs are
  `e2e-tauri/*.wdio.ts` (a suffix neither Playwright nor `bun test` picks up).
  They assert DOM render AND drive real IPC over the actual binary,
  `browser.tauri.execute(({core}) => core.invoke(...))` round-trips a command,
  `browser.tauri.mock(...)` stubs one (the desktop analogue of Playwright's
  `page.route`). The build enables `withGlobalTauri` test-only (via `TAURI_CONFIG`
  in `build-wdio-app.ts`, never `tauri.conf.json`) so the wdio plugin can reach
  the Tauri API. `@wdio/tauri-service` + `-plugin` are pinned to `1.2.0`, which
  can't import as published, its `dist` references `installMockSyncOverride`
  from `@wdio/native-utils` but the published `native-utils@2.4.0` predates that
  symbol (upstream added it in webdriverio/desktop-mobile PR #441 without
  republishing native-utils). A committed `bun patch`
  (`patches/@wdio%2Fnative-utils@2.4.0.patch`) ports the symbol in; it retires
  once upstream republishes native-utils with the bump.
- **Cross-engine sharing**: `e2e-shared/` holds a tiny runner-agnostic `UiDriver`
  + curated web-editor flows; `cross_engine.e2e.ts` (Playwright/Chromium) and
  `cross_engine.wdio.ts` (WDIO/WebKitGTK) each wrap their API and run the same
  bodies, so a WebKit-only render/JS divergence shows up as a desktop-only fail.
  Keep shared flows to render/interaction (no file-chooser, no backend stub).
