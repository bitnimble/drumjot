"""DilatedTransformerLayer — vendored from Beat-Transformer/code/DilatedTransformerLayer.py.

Only modification: removed the `__main__` test block. All math and tensor
shapes are byte-identical to upstream; do NOT refactor — the released
checkpoints were trained against this exact computation graph (including
the known shift-2 bug noted inline in `forward`).
"""
...
class DilatedMultiheadSelfAttentionWithRelativePositionalEmbedding(nn.Module):
    ...
    def forward(self, query, key, value, layer=0):
        ...
        q = self.query(query).reshape(batch, time, self.num_heads, 1, self.head_dim).transpose(1, 2)
        k = self.key(key).reshape(batch, time, self.num_heads, 1, self.head_dim).transpose(1, 2)
        v = self.value(value).reshape(batch, time, self.num_heads, 1, self.head_dim).transpose(1, 2)
        ...
        k = torch.cat(
            (
                ...
                # Upstream comment: "This should be k[:, 7: 8]. We had this
                # bug during training so we keep it to fit the checkpoints."
        ...
        Er_t = self.Er.unsqueeze(1).unsqueeze(0)
    ...
    def kv_roll(self, tensor, layer, padding_value=0, shift=1):
        ...
        tensor = F.pad(
            tensor,
            (0, 0, 0, 0, (2**layer) * (self.attn_len // 2), (2**layer) * (self.attn_len // 2)),
            mode="constant",
            value=padding_value,
        )
        ...
        tensor = torch.cat(
            [
                torch.roll(tensor, shifts=-i * (2**layer), dims=2)
...
class DilatedTransformerLayer(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.self_attn = DilatedMultiheadSelfAttentionWithRelativePositionalEmbedding(
            d_model, nhead, dropout, Er_provided, attn_len
        )
