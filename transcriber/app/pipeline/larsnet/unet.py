# Vendored verbatim from polimi-ispl/larsnet (`unet.py`).
#
# Source: https://github.com/polimi-ispl/larsnet
# Paper:  A. I. Mezza et al., "Toward Deep Drum Source Separation",
#         arXiv:2312.09663 / Pattern Recognition Letters 183 (2024).
#
# The upstream repo ships no pip package, so the model architecture is
# vendored here unchanged. The PRETRAINED WEIGHTS loaded into these
# modules are licensed CC BY-NC 4.0 (non-commercial); see
# `app/pipeline/larsnet/__init__.py` and `provision.py` for the source
# and the licensing constraint that carries onto any deployment shipping
# them.
#
# Kept byte-for-byte identical to upstream so it can be re-synced; all
# Drumjot-specific loading/separation glue lives in `__init__.py`, never
# in this file.
import math
import torch
