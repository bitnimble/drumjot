...
here, renamed to `beat_transformer.pt`. The Dockerfile copies this
directory into the image at build time, and `pipeline/beat_transformer.py`
loads `/app/checkpoints/beat_transformer.pt` at first inference.
...
`docker-compose.yml` (or the `.env` file).
