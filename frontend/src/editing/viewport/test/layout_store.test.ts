...
function jotWith(notes: Record<string, { lane: string; beat: number }>, bars = 1) {
...
describe('LayoutStore', () => {
  it('density factor is 1 at the reference onset density (2 onsets/beat)', () => {
    ...
    const notes: Record<string, { lane: string; beat: number }> = {};
    for (let i = 0; i < 8; i++) notes[`n${i}`] = { lane: 'k', beat: i * 0.5 };
  ...
  it('clamps sparse and dense densities', () => {
    const sparse = layout(jotWith({ a: { lane: 'k', beat: 0 } })); // 1 onset / 4 beats = 0.25 → /2 = 0.125 → floor 0.4
    expect(sparse.densityFactor).toBeCloseTo(0.4);
  });

  it('lays bars out left-to-right with cumulative x', () => {
    const l = layout(jotWith({ a: { lane: 'k', beat: 0 }, b: { lane: 'k', beat: 2 } }, 2));
  ...
  it('notePadPx scales the engraving inset by pxPerBeat', () => {
    const model = jotWith({ a: { lane: 'k', beat: 0 } });
