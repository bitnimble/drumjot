...
_CHART = {
    ...
    "events": [
        ...
        {"name": "BP_China15_C_1", "time": 2.5},  # -> misc cymbal
...
def test_comparison_folds_group_when_chart_has_one_subclass():
    ...
    est = {"hc": [0.5], "ho": [0.6], "rd": [1.0], "cr": [1.1], "mc": [1.2], "k": [0.0]}
    ...
    # cymbals folded to "cym": model rd+cr+mc vs the single chart cymbal
    assert "rd" not in p and "cr" not in p
    assert p["cym"] == ([1.0], [1.0, 1.1, 1.2])
